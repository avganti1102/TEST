...
function detailGroup(id){
    ...
    var idGroup = departments[index].id;
    var url = new URL("http://localhost:8080/api/v1/groups/" + idGroup);
    ...
    $.get(url, function(data, status) {
        ...
        parseData(data);
        $('p').append(
            '<h1 style="text-align:center;">' + data.name + '<a href="#editEmployeeModal" class="edit" onclick="openUpdateModal(' + data.id + ')"><i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></a></h1><br>' +
            '<b>Creator: </b>' + data.userName + '<br>' +
            '<b>Create Date: </b>' + data.createDate
        );
...
function Department(id, name, userName, createDate) {
    this.id = id;
    this.name = name;
...
function callAPI() {
    ...
    to = $('#to').val();
    search = $('#search').val();
    var url = new URL("http://localhost:8080/api/v1/groups/");
    if(from != null){
        if(to == null){
...
function fillDepartmentToTable() {
    departments.forEach(function(item) {
        $('tbody').append(
            ...
            '</span>'+
           '</td>' +  
            '<td><a href="#" onclick="detailGroup( ' +item.id + ')" >' + item.name + '</a></td>' +
            '<td>' + item.userName + '</td>' +
            ...
            '<a href="#deleteEmployeeModal" class="delete" onClick="openConfirmDelete(' + item.id + ')"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i></a>'  +   
            '</td>' +
            '</tr>')
    });
    $('tbody').append(
...
function openUpdateModal(id) {
    var url = new URL("http://localhost:8080/api/v1/groups/" + id);
    // call API from server
    $.get(url, function(data, status) {
    ...
    // fill data
    document.getElementById("id").value = departments.id;
    document.getElementById("name").value = departments.name;
